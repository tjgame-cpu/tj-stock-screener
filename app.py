...
import pandas as pd
from screener import CURATED_UNIVERSES, DB_NAME, StockScreeningEngine, init_db, update_past_predictions
import sqlite3
...
# Persistent file path to preserve scan data across browser reloads
CACHE_FILE = "latest_scan_cache.pkl"


def load_cached_results():
  """Loads the most recently saved scan from disk if it exists."""
...
def save_cached_results(df):
  """Persists scan output so reloads or browser restarts do not wipe data."""
  try:
...
<style>
    ...
    /* Native Code Block Contrast Fix for Light Mode */
    code, pre {
        background-color: #F8FAFC !important;
        color: #0F172A !important;
        border: 1px solid #CBD5E1 !important;
...
# Assemble initial watchlist pool
ALL_INBUILT_TICKERS = list(
    dict.fromkeys(
...
# Restore scan data from disk cache if session state is empty (prevents vanishing on reload)
if "scan_results" not in st.session_state:
  st.session_state.scan_results = load_cached_results()
...
st.sidebar.caption(
    "Tap below to load all Heavyweights, Midcaps, and Liquid ETFs:"
)
...
# Execute scan, update session state, and persist to local cache file
if run_scan:
  if not parsed_tickers:
    st.sidebar.warning("No tickers found in the queue.")
  else:
    with st.status(
        f"⚡ Running Screening Engine on {len(parsed_tickers)} queued"
        " assets...",
        expanded=True,
    ) as status:
      ...
      save_cached_results(results_df)

      status.update(
          label="✅ Quantitative Analysis Complete!",
          state="complete",
          expanded=False,
      )
...
with tab1:
  ...
  if results_df is not None and not results_df.empty:
    # Display last saved time badge if cache file exists
    cache_info = ""
    if os.path.exists(CACHE_FILE):
      saved_time = datetime.fromtimestamp(
          os.path.getmtime(CACHE_FILE)
      ).strftime("%d-%b %I:%M %p")
      cache_info = f" • Cached: {saved_time}"

    # --- INTERACTIVE FILTER CONTROLS ---
    st.markdown(f"### 🎛️ Screener Filters <span style='font-size:0.8rem; font-weight:normal; color:#64748B;'>({cache_info})</span>", unsafe_allow_html=True)
    ...
    with f_col4:
      filter_conf = st.selectbox(
          "ML Confidence:", options=["All", "≥ 50%", "≥ 60%", "≥ 70%"], index=0
      )
    ...
    if filter_flow == "Accumulation Only":
      filtered_df = filtered_df[
          filtered_df["FII/DII Institutional Flow"].str.contains(
              "Accumulation", case=False, na=False
          )
      ]
    elif filter_flow == "Distribution Only":
      filtered_df = filtered_df[
          filtered_df["FII/DII Institutional Flow"].str.contains(
              "Distribution", case=False, na=False
          )
      ]
    elif filter_flow == "Normal Rotation":
      filtered_df = filtered_df[
          filtered_df["FII/DII Institutional Flow"].str.contains(
              "Rotation", case=False, na=False
          )
      ]
    ...
    # Separate filtered items into Stocks vs ETFs
    is_etf = filtered_df["RawTicker"].isin(
        CURATED_UNIVERSES["Top 50 Liquid ETFs"]
    ) | filtered_df["Ticker"].str.contains(
        "BEES|ETF|MON100|MAFANG", case=False, regex=True
    )
    stocks_df = filtered_df[~is_etf].copy()
    etfs_df = filtered_df[is_etf].copy()

    # --- 1-TAP SHORTLIST COPY BLOCK ---
    st.markdown("#### 📋 Copy Shortlisted Tickers")
    copy_tab1, copy_tab2, copy_tab3 = st.tabs([
        f"All Shortlisted ({len(filtered_df)})",
        f"Equities Only ({len(stocks_df)})",
        f"ETFs Only ({len(etfs_df)})",
    ])

    with copy_tab1:
      all_str = (
          ", ".join(filtered_df["Ticker"].tolist())
          if not filtered_df.empty
          else "No instruments match current filters."
      )
      st.code(all_str, language="text")
      st.caption("Tap the copy icon on the top right of the box to copy.")

    with copy_tab2:
      stocks_str = (
          ", ".join(stocks_df["Ticker"].tolist())
          if not stocks_df.empty
          else "No stocks match current filters."
      )
      st.code(stocks_str, language="text")

    with copy_tab3:
      etfs_str = (
          ", ".join(etfs_df["Ticker"].tolist())
          if not etfs_df.empty
          else "No ETFs match current filters."
      )
      st.code(etfs_str, language="text")

    st.markdown("---")

    # --- KITE WATCHLIST CARD RENDERER ---
    def render_watchlist_cards(df, title_label):
      ...
      for _, row in df.iterrows():
        ...
        with st.expander(f"📊 Deep Analysis for {row['Ticker']}"):
          ...
          c2.metric(
              "Institutional Volume (FII/DII)", row["FII/DII Institutional Flow"]
          )
          c3.metric("Relative Volume Footprint", f"{row['RVol']}x Average")
          st.markdown(
              f"👉 **[Open {row['Ticker']} Live Chart on TradingView"
              f" ↗]({row['TradingView Link']})**"
          )
    ...
    st.info(
        "👈 Tap **'⚡ Run Inbuilt Market Finder'** in the sidebar, then tap"
        " **'🚀 Run Quantitative Scan'** to analyze all stocks and ETFs."
    )
...
with tab2:
  ...
  try:
    ...
    if not ledger_df.empty:
      ledger_df["ticker"] = (
          ledger_df["ticker"].str.replace(".NS", "").str.replace(".BO", "")
      )

      def highlight_status(val):
        if val == "Hit":
          return (
              "background-color: #E6F7EF; color: #00A25B; font-weight: bold;"
          )
        if val == "Missed":
          return (
              "background-color: #FDF0EE; color: #DF514C; font-weight: bold;"
          )
        return "background-color: #FFF7ED; color: #C2410C; font-weight: bold;"

      st.dataframe(
          ledger_df.style.map(
              highlight_status, subset=["status_short", "status_long"]
          ),
...
with tab3:
  ...
  st.write(
      "Simulates entry signals and reward-to-risk performance over historical"
      " daily candles."
  )
  ...
  if st.button("🔬 Execute Backtest Simulation"):
    with st.spinner("Processing historical backtest cycles..."):
      engine = StockScreeningEngine(
          parsed_tickers[:15]
          if parsed_tickers
          else CURATED_UNIVERSES["Nifty 50 Heavyweights"][:10]
      )
      engine.fetch_data()
      ...
      if not bt_df.empty:
        ...
        c1, c2 = st.columns(2)
        c1.metric(
            "Average Peak Simulated Upside", f"+{round(avg_gain, 2)}%"
        )
